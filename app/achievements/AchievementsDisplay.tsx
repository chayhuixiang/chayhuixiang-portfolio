...
const Achievements = ({ fetchedAchievements }: Props) => {
  ...
  const filteredAchievement = fetchedAchievements.find(
    (achievement) => achievement.id === selectedAchievement
  );
  ...
  return (
    <div className="flex flex-col md:flex-row justify-between">
      <div className="relative">
        ...
        <p className="mt-2 mb-7 hidden md:block">
          Here is a list of my notable achievements.
        </p>
        <div className="flex flex-col flex-1 gap-3 md:gap-7 items-center md:items-start mt-3 md:mt-0">
          {fetchedAchievements.map(
            ({ name, id, position, year, logo_path }) => (
              <div key={id} className="w-full">
                <h3
                  className={`hidden md:block font-bold cursor-pointer hover:text-gunmetal dark:hover:text-white ${
                    id === selectedAchievement
                      ? "text-gunmetal dark:text-white"
                      : "text-blue-dark dark:text-zinc-300"
                  }`}
                ...
                <div
                  className="bg-zinc-100 dark:bg-purple-dark p-3 md:hidden w-full rounded-xl flex gap-3 items-center whitespace-nowrap"
                  onClick={() => setSelectedAchievement(id)}
                >
        ...
        <Portal>
          <div
            className={`flex items-center justify-center px-4 w-screen h-[100%] bg-white dark:bg-indigo fixed left-0 top-0 z-40 transition-all ${
              selectedAchievement
                ? "visible bg-opacity-80 opacity-100"
                : "invisible bg-opacity-0 opacity-0"
            } md:invisible md:opacity-0`}
            ref={ref}
          >
            {filteredAchievement && (
              <div
                className="bg-white dark:bg-purple-dark shadow-around-lg w-[min(25rem,100%)] px-4 py-3 rounded-xl flex flex-col items-center gap-4"
                ref={modalRef}
              >
                <div className="flex justify-between h-8 w-full items-center">
                  <XMarkIcon
                    className="text-gunmetal dark:text-white w-6 h-6 stroke-2"
                    onClick={() => setSelectedAchievement("")}
                  />
                  <p className="font-bold">
                    {filteredAchievement.name} {filteredAchievement.year}
                  </p>
                  <a
                    href={filteredAchievement.link}
                ...
                <div className="flex items-center gap-[0.375rem]">
                  <div className="rounded-full bg-white shadow-around-lg flex items-center justify-center aspect-square w-9">
                    <img
                      src={filteredAchievement.logo_path}
                      alt="achievement_logo"
                    />
                  </div>
                  <div className="flex flex-col">
                    ...
                    <p className="text-xs font-secondary">
                      {new Date(
                        filteredAchievement.start_date
                      ).toLocaleDateString("en-GB", {
                        month: "short",
                        year: "numeric",
                      })}{" "}
                      –{" "}
                      {filteredAchievement.end_date === null
                        ? "Present"
                        : new Date(
                            filteredAchievement.end_date
                          ).toLocaleDateString("en-GB", {
                            month: "short",
                            year: "numeric",
                          })}
                ...
                <p className="text-xs text-left mx-3">
                  {filteredAchievement.description
                    .split("*")
                    .map((descriptionChunk, i) =>
                      i % 2 ? (
                        <strong key={i}>{descriptionChunk}</strong>
                      ) : (
                        <span key={i}>{descriptionChunk}</span>
                      )
                    )}
                </p>
                <img
                  src={filteredAchievement.cover_path_mobile}
                  alt="achievement_cover"
      ...
      <div className="hidden md:block w-[33.313rem]">
        {filteredAchievement && (
          <motion.div
            className="flex flex-col px-7 gap-8 transition-all duration-200"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          >
            <img
              src={filteredAchievement.cover_path_desktop}
              alt="achievement_cover"
              className="shadow-lg rounded-xl"
            />
