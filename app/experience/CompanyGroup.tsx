  ...
  return (
    <>
      ...
      <div className='w-full mt-4 sm:mt-8 md:mt-32 flex flex-row gap-4 overflow-x-scroll pb-8 md:pb-32 snap-x snap-mandatory scrollbar scrollbar-track-zinc-100 dark:scrollbar-track-purple-dark scrollbar-thumb-purple dark:scrollbar-thumb-purple-light' ref={ref} onScroll={scrollHandler}>
        {sortedCompanies.map(({name, position, logo_path_light, logo_path_dark, start_date, end_date, description, stacks, link}, i) => 
          <CompanyCard key={i} focus={calculateMobileFocus(i)} name={name} position={position} logo_path_light={logo_path_light} logo_path_dark={logo_path_dark} start_date={start_date} end_date={end_date} description={description} stacks={stacks} link={link} />
        )}
      </div>
    </>
  )
