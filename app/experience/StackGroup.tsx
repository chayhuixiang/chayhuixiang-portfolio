...
const StackGroup = ({sortedWorkStack}: Props) => {
  ...
  return (
    <div className='grid xl:grid-cols-9 sm:grid-cols-5 grid-cols-3 gap-7'>
      {sortedWorkStack.map(({name, logo_path_light, logo_path_dark, colour, skill_work_order}: Stack) => 
        <StackCard name={name} logo_path_light={logo_path_light} logo_path_dark={logo_path_dark} colour={colour} key={skill_work_order} />)}
    </div>
  )
}
