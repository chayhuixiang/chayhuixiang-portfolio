    ...
    <div className="w-full max-w-7xl px-4 sm:px-[3rem] lg:px-[7.5rem] py-6 sm:py-20 m-auto flex flex-col items-center">
        ...
        <div className="flex gap-4 w-full md:w-auto">
          {buttons.map((buttonObj, i) => (
            <Button
              ...
              colour="blue-dark"
              type={buttonObj.name === selectedOption ? "solid" : "outline"}
          ...
          <Button
            ...
            colour="blue-dark"
            type="outline"
            className="mx-auto sm:mx-0 md:hidden w-[min(100%,400px)] justify-between"
          >
      ...
      <div className="mt-4 sm:mt-8 md:mt-20 grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-10 mb-3 sm:mb-10 w-[min(100%,400px)] sm:w-full">
        {displayedProjects.map(
          (
            { name, colour, cover_path, deployed_link, repo, owner, stacks },
            i
          ) => (
            <RepoCard
              key={i}
              variant={
                i === 0 && displayedProjects.length % 2 === 1
                  ? "large"
                  : "small"
              }
              name={name}
