...
const RepoCard = ({
  ...
  return (
    <article
      className={`p-4 bg-white dark:bg-indigo rounded-xl ${
        variant === "large" ? "sm:row-span-2" : "sm:row-span-1"
      }`}
      style={{}}
    ...
    >
      <div
        style={{
          backgroundColor: `${hovered ? colour + "4D" : colour + "33"}`,
          backgroundImage: `url(${cover_path})`,
        }}
        className={`aspect-[64/79] ${
          variant === "large"
            ? "sm:aspect-auto sm:h-full"
            : "sm:aspect-[117/92]"
        } max-w-[29.25rem] w-full bg-cover rounded-xl p-6 transition-all duration-200 flex flex-col items-start`}
      >
        <div className="flex justify-between w-full whitespace-nowrap gap-4">
          <h2 className="font-bold text-ellipsis overflow-hidden text-2xl md:text-[2rem] dark:text-zinc-300">
            {name}
          </h2>
        ...
        <div className="flex justify-between w-full mt-2 md:mt-4">
          ...
          <div className="flex sm:hidden gap-2 min-w-max items-center">
            ...
            >
              <img
                src="/images/projects/repo-github.svg"
                alt="github_link"
                className="w-6 md:w-8"
              />
            </a>
            {deployed_link && (
              ...
              >
                <img
                  src="/images/projects/repo-deployed.svg"
                  alt="deployment_link"
                  className="w-6 md:w-8"
                />
              </a>
